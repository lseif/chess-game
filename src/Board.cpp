...
void Board::unruledMove(const Move& move) {
  ...
  if(isKing(*move.getPiece())) {
    auto it = std::find(kingPositions[move.getPiece()->color].begin(), kingPositions[move.getPiece()->color].end(), move.getPiece()->position);
    if(it!=kingPositions[move.getPiece()->color].end()){
      kingPositions[move.getPiece()->color].erase(it);
    }

    kingPositions[move.getPiece()->color].push_back(move.getPosition()); //update king position
  }
...
void Board::placeNewPiece(Piece& piece) {
  ...
  if(isKing(piece)) {
    auto it = std::find(kingPositions[piece.color].begin(), kingPositions[piece.color].end(), piece.position);
    if(it!=kingPositions[piece.color].end()){
      kingPositions[piece.color].erase(it);
    }

    kingPositions[piece.color].push_back(piece.position); //update king position
  }
        ...
        if(!kingPositions[move.getPiece()->color].empty()) {
          ...
          if(pieceAttackingKing.has_value()) {
            ...
            if(promotion) {//revert promotion
              std::shared_ptr<Piece> pawn = std::shared_ptr<Piece>(Pawn(move.getPiece()->color, move.getPiece()->position).clone());
              placePiece(*pawn);
            }
...
void Board::castle(const bool& direction, const Move& move) {
  ...
  if (direction==SHORT) {
    if(!pieceAt(Pos(move.getPiece()->position.x, BOARDSIZE-1))->isMoved&&!move.getPiece()->isMoved) {
      for (int i = move.getPiece()->position.y+1; i < BOARDSIZE-1; i++) {
        if(isEmpty(Pos(move.getPiece()->position.x, i))) {
          if(i-1<move.getPiece()->position.y+2) {//-1 to check if king is in check. Castling is not allowed if King is in Check
            if (isCovered(Pos(move.getPiece()->position.x, i-1), !move.getPiece()->color)) {
              throw NotAllowedMoveException(("cant castle." + Pos(move.getPiece()->position.x, i-1).show() + " is covered \n").c_str());
            }
          }
          castle=true;

        } else {
          throw NotAllowedMoveException(("cant castle." + Pos(move.getPiece()->position.x, i).show() + " isn't empty \n").c_str());
        }
      }
    } else {
      throw NotAllowedMoveException(("cant castle. you already moved one of the two pieces\n"));
    }
    if(castle) {
      Move rookMove = Move(*pieceAt(Pos(move.getPiece()->position.x, BOARDSIZE-1)), Pos(move.getPiece()->position.x, move.getPiece()->position.y+1));
      ...
      for (int i = move.getPiece()->position.y-1; i >0; i--) {
        if(isEmpty(Pos(move.getPiece()->position.x, i))) {
          if(i>move.getPiece()->position.y-2) {
            if (isCovered(Pos(move.getPiece()->position.x, i), !pieceAt(move.getPiece()->position)->color)) {
              throw NotAllowedMoveException(("cant castle." + Pos(move.getPiece()->position.x, i).show() + " is covered \n").c_str());
            }
          }
          castle=true;
        } else {
    ...
    if(castle) {
      Move rookMove = Move(*pieceAt(Pos(move.getPiece()->position.x, 0)), Pos(move.getPiece()->position.x, move.getPiece()->position.y-1));
      unruledMove(rookMove);
      Move kingMove = move;
      unruledMove(kingMove);
    }
